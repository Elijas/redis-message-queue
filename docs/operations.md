...
| Completed log | list | `name::completed` | `queue.key.completed` | `queue.stats().completed`, `queue.peek(source="completed")` first | Written when `max_completed_length` is positive or `None`; `0` disables tracking without deleting existing entries. |
| Failed log | list | `name::failed` | `queue.key.failed` | `queue.stats().failed`, `queue.peek(source="failed")` first | Written when `max_failed_length` is positive or `None`; `0` disables tracking without deleting existing entries. |
...
- **Setting `get_deduplication_key=None` disables deduplication with a retention-window overlap.** Existing dedup records remain in Redis until their TTL expires, but new publishes bypass them. Republishes that would have been suppressed under the old setting can enqueue during that window.
- **Setting `max_failed_length=0` stops recording handler failures.** Existing failed entries remain in Redis, but new failures are removed from `processing` without being appended to the failed queue. If `max_delivery_count=None` is also set, repeated handler failures can be dropped with no DLQ or failed-queue record; see [Dead-letter queue](configuration.md#dead-letter-queue).
- **Setting a smaller positive `max_completed_length` or `max_failed_length` trims existing history.** The next completed or failed move calls `LTRIM`, so changing `None` to a positive `N` or lowering a positive cap can reduce historical entries to the new cap. Setting either limit to `0` disables that tracking list without deleting or trimming its existing entries.
