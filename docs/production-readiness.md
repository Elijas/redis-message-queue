...
| Deduplication key generation | MITIGATED | Deduplication is opt-in and requires an explicit `get_deduplication_key` whenever `deduplication=True`, so no implicit "every distinct payload" dedup keyspace can be created accidentally. The callable must return a non-empty string; `None`, `""`, and non-`str` returns fail before enqueue. | [Configuration: deduplication](configuration.md#deduplication), `test_dedup_empty_key.py` |
