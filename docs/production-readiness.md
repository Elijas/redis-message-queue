...
Before changing constructor parameters on a live queue, see
[Configuration changes on live queues](operations.md#configuration-changes-on-live-queues) —
several parameter changes are destructive on populated queues.
...
| Poison message redelivery | MITIGATED | Messages exceeding the `max_delivery_count` delivery limit are routed to a dead-letter queue instead of being redelivered indefinitely. On the built-in `client=` path, omitting `max_delivery_count` uses the capped default of `10`; set `max_delivery_count=None` explicitly to disable dead-lettering and allow unlimited redelivery. | `test_dead_letter.py`, `test_lease_stress.py:TestPoisonMessageIsolation` |
| Completed and failed queue retention | MITIGATED | Tracking is disabled by default (`max_completed_length=0`, `max_failed_length=0`). Positive limits enable tracking and call LTRIM after each move to cap list growth; set `max_completed_length=None` / `max_failed_length=None` explicitly to enable unbounded retention. Setting either limit to `0` leaves existing history untouched. | [Configuration: success and failure tracking](configuration.md#success-and-failure-tracking), `test_process_message.py:TestBoundedCompletedQueue`, `test_process_message.py:TestBoundedFailedQueue` |
...
| Connection pool sizing | MEDIUM | **redis-py's default standalone client `max_connections=None` resolves to `100` and raises `ConnectionError` once the pool is exhausted.** Pass `max_connections=<finite>` to `redis.Redis()` sized to expected worker + heartbeat concurrency. | [Configuration: connection pool sizing](configuration.md#connection-pool-sizing) |
...
| Deduplication key generation | MITIGATED | Deduplication is enabled by supplying an explicit `get_deduplication_key` callable, so no implicit "every distinct payload" dedup keyspace can be created accidentally. The callable must return a non-empty string; `None`, `""`, and non-`str` returns fail before enqueue. | [Configuration: deduplication](configuration.md#deduplication), `test_dedup_empty_key.py` |
...
- `RedisMessageQueueError` (base)
  ...
  - `RetryBudgetExhaustedError` - also a subclass of `redis.RedisError` so Redis-layer handlers see retry exhaustion
...
- Redis Cluster runtime coverage is partial, not absent. Construction-time hash-tag validation is tested, and `test_integration_cluster.py` exercises hash-tagged multi-key Lua evaluation (publish/ack/reclaim, sync and async) plus cross-slot `pcall`-rejection paths against a real `RedisCluster` — but only when `REDIS_CLUSTER_URL` is set (its dedicated `real-redis-cluster` CI job), skipping on local runs without a cluster. `test_cluster_moved_redirect.py` pins MOVED/ASK redirect handling by *simulating* a redirect at the gateway's script-eval boundary (an injected `MovedError`/`AskError` on single-node Redis): a redirect that escapes redis-py surfaces as a catchable, non-retryable `ResponseError`, is not rewrapped as `LuaScriptError`, and leaves the lease/claim bookkeeping intact so a settled-migration retry succeeds. `test_integration_cluster_resharding.py` (same CI job) now exercises a *live* slot migration on the real multi-node cluster: with a lease outstanding it runs the full `CLUSTER SETSLOT` + `MIGRATE` resharding handshake, then drives lease renewal, ack, and a fresh publish/claim through the gateway — pinning that redis-py transparently refreshes its slot map and re-routes *below* `client.eval` with no error reaching the gateway. What still lacks a real-cluster fixture: redirects during *partial or failed* migrations (ASK transitional states while a slot is mid-handshake with keys split across nodes), multi-slot resharding storms, and the degraded case where a bare redirect reaches the gateway only after redis-py exhausts its own redirect retries — that last path remains covered only by the single-node simulation.
