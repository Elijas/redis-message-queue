...
| R9 | LOW | Dead-letter queue grows without bound — no `max_dead_letter_length` parameter exists. Under sustained poison-message load on the built-in `client=` path, monitor the default DLQ via `LLEN {name}::dlq` and trim manually if needed. Custom gateways can choose a different `dead_letter_queue`; monitor the configured key instead. | [README dead-letter queue](../README.md#dead-letter-queue), `test_dead_letter.py` |
