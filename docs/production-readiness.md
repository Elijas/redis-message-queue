...
| R9 | LOW | Dead-letter queue grows without bound — no `max_dead_letter_length` parameter exists. Under sustained poison-message load on the built-in `client=` path, monitor the default DLQ via `LLEN {name}::dlq`. Before trimming, inspect the configured DLQ, export/archive retained raw-payload records as needed, then trim only according to an application-owned retention policy. Custom gateways can choose a different `dead_letter_queue`; monitor and apply that policy to the configured key instead. | [README dead-letter queue](../README.md#dead-letter-queue), `test_dead_letter.py` |
...
| R14 | LOW | B2 `max_pending_length` does not cap completed/failed lists; those have separate caps via `max_completed_length` / `max_failed_length`. | [README publish backpressure](../README.md#publish-backpressure), [README success/failure tracking](../README.md#success-and-failure-tracking) |
