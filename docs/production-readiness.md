...
| R6 | MITIGATED | ~~No metrics or observability hooks~~ — **mitigated** by the `on_event` constructor callback, which receives a `QueueEvent` dataclass for publish/claim/ack/reclaim/dedup/cleanup/drain lifecycle events. Remaining caveats: callbacks are best-effort; callback exceptions are logged/warned but do not crash queue operations; the library does not ship metrics exporters. Use `examples/production/observability.py` as the adapter pattern. | [Observability guide](observability.md), `examples/production/observability.py` |
...
| R10 | MEDIUM | Consumer hang with heartbeat keeps message locked forever — when handler code hangs, heartbeat renews the lease indefinitely. No processing-time cap exists. Monitor processing-queue dwell time externally. | `_LeaseHeartbeat._run` loop, configuration.md (crash recovery tradeoffs) |
...
| R14 | LOW | B2 `max_pending_length` does not cap completed/failed lists; those have separate caps via `max_completed_length` / `max_failed_length`. | [Configuration: publish backpressure](configuration.md#publish-backpressure), [Configuration: success and failure tracking](configuration.md#success-and-failure-tracking) |
...
| R16 | LOW | B5/AC-03 `drain()` / `aclose()` do not cancel in-flight handlers that started before drain; handlers must reach natural completion. Explicit drain now refuses new publishes on the same queue instance with `QueueDrainedError`. | [Configuration: graceful shutdown](configuration.md#graceful-shutdown) |
| R17 | LOW | B10 callback exceptions are caught, logged, and emitted as a queue warning; they do not interrupt queue operations. | [Observability guide](observability.md) |
| R18 | LOW | B10/AC-03 queue-specific failures share `RedisMessageQueueError`; publish after explicit drain raises `QueueDrainedError`. See `redis_message_queue._exceptions` for the active hierarchy. | [Observability guide](observability.md) |
| R19 | MEDIUM | **redis-py default standalone client `max_connections=None` resolves to `2**31`. A concurrency spike retains spike-created sockets until client close.** Pass `max_connections=<finite>` to `redis.Redis()` sized to expected worker + heartbeat concurrency. (Source: R7 AC-12 F1) | [Configuration: connection pool sizing](configuration.md#connection-pool-sizing) |
| R20 | LOW | **Fork after construct is unsupported for queue/client objects.** Construct queue + Redis client after fork in worker processes. Sync pooled Redis recovers via redis-py PID-reset, but async clients do not. If a parent already installed `GracefulInterruptHandler`, a child can call `reset()` on the inherited handler before constructing its own handler. (Source: R7 AC-10) | [Operations: fork safety](operations.md#fork-safety-and-pre-fork-servers) |
