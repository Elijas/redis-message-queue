...
class TestProcessMessageStaleLease:
    ...
    def test_cleanup_double_fault_emits_runtime_warning(self):
        gateway = FakeGateway()
        gateway.message_to_return = ClaimedMessage(stored_message=b"msg", lease_token="tk1")
        gateway.fail_on_remove = True
        queue = RedisMessageQueue("test", gateway=gateway)
    ...
    def test_trim_failure_emits_runtime_warning(self):
        gateway = FakeGateway()
        gateway.message_to_return = b"msg"
...
class TestClusterHashTagCompatibility:
    ...
    def test_visibility_timeout_claim_uses_only_colocated_keys_without_dead_letter(self):
        ...
        gateway._claim_visible_message("{myqueue}::pending", "{myqueue}::processing", claim_id="claim-1")
...
class TestBoundedCompletedQueue:
    ...
    def test_completed_queue_is_trimmed(self):
        client = fakeredis.FakeRedis()
        gateway = RedisGateway(
            redis_client=client,
            retry_budget_seconds=0,
        ...
        assert client.llen(queue.key.completed) == max_len
        assert client.llen(queue.key.processing) == 0
        assert client.llen(queue.key.pending) == 0

    def test_no_trim_without_max_completed_length(self):
        client = fakeredis.FakeRedis()
        gateway = RedisGateway(
            redis_client=client,
            retry_budget_seconds=0,
...
class TestBoundedFailedQueue:
    ...
    def test_failed_queue_is_trimmed(self):
        client = fakeredis.FakeRedis()
        gateway = RedisGateway(
            redis_client=client,
            retry_budget_seconds=0,
        ...
        assert client.llen(queue.key.failed) == max_len
        assert client.llen(queue.key.processing) == 0
        assert client.llen(queue.key.pending) == 0

    def test_no_trim_without_max_failed_length(self):
        client = fakeredis.FakeRedis()
        gateway = RedisGateway(
            redis_client=client,
            retry_budget_seconds=0,
