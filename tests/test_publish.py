...
class TestPublishWithDeduplication:
    ...
    def test_v7_compatible_dict_dedup_key_uses_canonical_hash(self, queue, redis_client):
...
class TestPublishMessageTypeValidation:
    def test_preflight_validation_failure_emits_publish_failure_event(self, redis_client):
        events = []
        queue = RedisMessageQueue(
            "test-queue",
            client=redis_client,
