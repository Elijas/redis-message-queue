"""Make a consumer idempotent so at-least-once redelivery is safe.
...
def do_side_effect(business_id: str, payload: dict) -> None:
...
def handle(client: Redis, message: str) -> None:
    ...
    won = client.set(key, "1", nx=True, ex=IDEMPOTENCY_TTL_SECONDS)
    if not won:
        log.info("duplicate id=%s already processed; skipping side effect", business_id)
        return

    try:
        do_side_effect(business_id, payload)
    except Exception:
...
def main() -> None:
    ...
    try:
        ...
        # three times.
        queue.publish({"id": "order-1", "body": "charge order 1"})
        queue.publish({"id": "order-2", "body": "charge order 2"})
        queue.publish({"id": "order-1", "body": "charge order 1 (duplicate)"})
